...
Console.WriteLine(r.stampaMaxAree(r.areeMaxTipi()));

Console.WriteLine(r.maxArea().Equals(r.minPerimetro()));
Console.WriteLine(r.L.First().Equals(5));
...
class Quadrato : Rettangolo
{
    ...
    public override bool Equals(object obj)
    {
        ...
        {
            Console.WriteLine("Cast non corretto");
            return false;
        }
    }
}
...
class Raccolta
{
    ...
    public List<FiguraGeometrica> L { get { return l; } }
    List<FiguraGeometrica> listaAree;
    int conta = 0;
    public Raccolta()
    {
        l = new List<FiguraGeometrica>();
        listaAree = new List<FiguraGeometrica>();
    }
    ...
    public List<FiguraGeometrica> areeMaxTipi()
    {
        string s = "";
        int conta = 0;
        FiguraGeometrica max = null;
        List<Type> listaTipi = new List<Type>();
        listaAree = new List<FiguraGeometrica>();
        foreach (FiguraGeometrica item in l)
        {
            if (!listaTipi.Contains(item.GetType()))
            {
                listaTipi.Insert(conta, item.GetType());
                conta++;
            }
        }
        foreach (Type item in listaTipi)
        {
            double conta2 = 0;
            foreach (FiguraGeometrica n in l)
            {
                if (item == n.GetType())
                {
                    if (conta2 < n.CalcolaArea())
                    {
                        conta2 = n.CalcolaArea();
                        max = n;
                    }
                }
            }
    ...
    public string stampaMaxAree(List<FiguraGeometrica> li)
    {
        string s = "";
        foreach (FiguraGeometrica item in listaAree)
        {
            s += item.ToString() + "\n----------------\n";
        }
        return s;
